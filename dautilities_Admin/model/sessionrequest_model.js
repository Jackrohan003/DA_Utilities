...
var sessionreq={
    getAllSessionReq:function(callback){
        return db.query("select * from sessionrequest",callback);
    },
    addSessionReq:function(item,callback){
        var date=new Date();
        return db.query("insert into sessionrequest (remailId,speakerName,sessionDetails,batchId,sessionTopic,sessionPrice,LocationId,sessionDate,sessionStatus,sessionAddedAt,sessionUpdatedAt) values (?,?,?,?,?,?,?,?,?,?,?)",[item.remailId,item.speakerName,item.sessionDetails,item.batchId,item.sessionTopic,item.sessionPrice,item.LocationId,item.sessionDate,item.sessionStatus,date,date],callback);
    },
    // updateSessionReq:function(id,item,callback){
    //     var date=new Date();
    //     return db.query("update sessionrequest set speakerName=?,sessionDetails=?,batchId=?,sessionTopic=?,sessionPrice=?,LocationId=?,sessionDate=?,sessionStatus=?,sessionUpdatedAt=? where sessionreqId=?",[item.speakerName,item.sessionDetails,item.batchId,item.sessionTopic,item.sessionPrice,item.LocationId,item.sessionDate,item.sessionStatus,date,id],callback);
    // },
